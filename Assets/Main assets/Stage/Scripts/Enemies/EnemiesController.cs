...
{
    private float SpawnSpeed_Alien = 20;
    private float SpawnSpeed_METEOROID = 5;
    private float LastSpawn_Alien = 0;
    private float LastSpawn_METEOROID = 0;

    const float MAX_SPAWN_SPEED_ALIEN = 10;
    const float MAX_SPAWN_SPEED_METEOROID = 2;
    const float SPAWN_COUNT_ALIENS = 1;
    const float SPAWN_COUNT_METEOROIDS = 5;
            ...
            !GameObject.Find("Terrain").GetComponent<StageEnvironment>().GetGameOverState())
        {
            ...
            if (time > LastSpawn_Alien + SpawnSpeed_Alien)
            {
                ...
                for (int i = 0; i < SPAWN_COUNT_ALIENS; i++)
                {
                    ...
                    enemy.name = "Alien";
                    enemy.transform.position = vec;
                    enemy.transform.GetComponent<Alien>().Init(0.15f);
